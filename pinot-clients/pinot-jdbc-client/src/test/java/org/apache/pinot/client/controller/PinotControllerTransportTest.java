...
public class PinotControllerTransportTest implements HttpHandler {
  ...
  public void validJsonResponse() {
    _responseJson = _VALID_RESPONSE_JSON;
    PinotControllerTransportFactory factory = new PinotControllerTransportFactory();
    PinotControllerTransport transport = factory.buildTransport();
    ControllerTenantBrokerResponse response = transport.getBrokersFromController("localhost:" + _dummyServer.getAddress().getPort(), "tenant123");
    List<String> brokers = response.getBrokers();
    assertEquals(brokers.size(), 2);
  }
